...
export default function DebugPanel() {
  ...
  useEffect(() => {
    ...
    const makeHandler =
      (level: string) =>
      (...args: unknown[]) => {
        original[level as keyof typeof original](...(args as Parameters<typeof console.log>));
        const msg = args
  ...
  useEffect(() => {
    ...
    fab.addEventListener('dragstart', preventDrag);
    ...
    return () => {
      ...
      fab.removeEventListener('dragstart', preventDrag);
    };
  }, [debugMode]);
  ...
  const filteredLogs =
    filterLevel === 'all' ? debugLogs : debugLogs.filter((e) => e.level === filterLevel);
  ...
  return (
    <>
      <div ref={fabRef} className="debug-fab">
        ...
        {debugLogs.length > 0 && (
          <span className="debug-fab-badge">
        ...
        <div className="debug-header" onClick={() => setIsOpen((o) => !o)}>
          <span className="debug-title">
            ...
            <span className="debug-count">
              ({filteredLogs.length}{filterLevel !== 'all' ? ` ${filterLevel}` : ''})
            </span>
          </span>
          <div className="debug-actions">
            ...
            >
              {copied
                ? <span style={{ fontSize: '0.625rem', color: '#4ade80' }}>Copied!</span>
                : <Icon svg={copyIconRaw} size={14} />}
            </button>
            <button
            ...
            <button className="debug-action-btn">
              <Icon svg={chevronDownIconRaw} size={14} />
            </button>
          </div>
        </div>
        ...
        <div className="debug-filter-bar" onClick={(e) => e.stopPropagation()}>
          {LEVELS.map((lvl) => (
            <button
              ...
              style={
                filterLevel === lvl && lvl !== 'all'
                  ? { borderColor: LEVEL_COLORS[lvl], color: LEVEL_COLORS[lvl] }
                  : {}
              }
            >
              {LEVEL_LABELS[lvl]}
              {lvl !== 'all' && counts[lvl] ? (
                <span className="debug-filter-count">{counts[lvl]}</span>
              ) : null}
            </button>
          ))}
        </div>
        ...
        <div ref={logsRef} className="debug-logs">
          {filteredLogs.length === 0 ? (
            <div className="debug-empty">
              No {filterLevel !== 'all' ? filterLevel : ''} logs yet.
            </div>
